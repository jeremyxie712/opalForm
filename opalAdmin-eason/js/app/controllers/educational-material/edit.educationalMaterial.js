angular.module('opalAdmin.controllers.educationalMaterial.edit', ['ngAnimate', 'ngSanitize', 'ui.bootstrap', 'ui.grid', 'ui.grid.expandable', 'ui.grid.resizeColumns','ui.sortable']).
	...
	controller('educationalMaterial.edit', function ($scope, $filter, $sce, $uibModal, $uibModalInstance, $state, educationalMaterialCollectionService, filterCollectionService, uiGridConstants, Session) {
		...
		educationalMaterialCollectionService.getEducationalMaterialDetails($scope.currentEduMat.serial).then(function (response) {
			...
			console.log($scope.eduMat);

			//copy selected value to trigger list
			// $scope.packageTriggerList = $scope.eduMat.packages;
			for(var i = 0; i< $scope.eduMat.packages.length; i++){
			    var edu = $scope.eduMat.packages[i];
				$scope.packageTriggerList.push({serial: Number(edu.MaterialsSerNum),name_en: edu.Name_EN, type_en: edu.Type_EN});
			}
		...
		$scope.updateEduMat = function () {
			...
			if ($scope.checkForm()) {
				...
				$scope.eduMat.packages = $scope.packageTriggerList;
		...
		$scope.checked = function(edu){
			var index= $scope.packageTriggerList.map(function(x){ return x.serial; }).indexOf(edu.serial);
		    return index!==-1;//true if exist
		};
