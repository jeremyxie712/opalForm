angular.module('opalAdmin.controllers.educationalMaterial.add', ['ngAnimate', 'ngSanitize', 'ui.bootstrap', 'ui.grid', 'ui.grid.resizeColumns','ui.sortable']).
	...
	*******************************************************************************/
	controller('educationalMaterial.add', function ($scope, $filter, $state, $sce, $uibModal, educationalMaterialCollectionService, filterCollectionService, Session) {
		...
		$scope.packageSection = {open:false, show:false};
		...
		//get existing education materials
		$scope.EduMaterials = [];
		...
		$scope.submitEduMat = function () {
			if ($scope.checkForm()) {
				...
				//add packages
				$scope.newEduMat.packages = $scope.packageTriggerList;

				// console.log($scope.newEduMat);
		...
		$scope.searchPackage = function (field){
			$scope.packageSearchField = field;
		...
		$scope.searchPackageFilter = function(Filter){
			var keyword = new RegExp($scope.packageSearchField, 'i');
			// console.log(Filter.name_EN);
			// console.log(keyword.test(Filter.name_EN));
			return !$scope.packageSearchField || keyword.test(Filter.name_FR) || keyword.test(Filter.name_EN);
		};
		...
		$scope.checkForm = function () {
			// console.log($scope.numOfCompletedSteps);
			// console.log(trackProgress($scope.numOfCompletedSteps, $scope.stepTotal));
		...
		$scope.isPackage = function () {
			if($scope.newEduMat.type_EN === "Package" && $scope.phaseSection.show){
				$scope.packageSection.show = true;
				$scope.urlSection.show = false;
				$scope.tocsSection.show = false;
				$scope.getEducationalMaterials();
			}else{
				$scope.packageSection.show = false;
				if($scope.phaseSection.show){
					$scope.urlSection.show = true;
					$scope.tocsSection.show = true;
				}
				$scope.packageTriggerList = [];
		...
		$scope.CheckMat = function(edu){
			console.log(edu);
			var index= $scope.packageTriggerList.map(function(x){ return x.serial; }).indexOf(edu.serial);
			if(index===-1){
				$scope.packageTriggerList.push({serial: edu.serial,name_en: edu.name_EN, type_en: edu.type_EN});
			}else{
				$scope.packageTriggerList.splice(index,1);
			}

			var plist = document.getElementById("plist");
			plist.scrollTop = plist.scrollHeight - plist.clientHeight;
			console.log(plist.scrollTop);
			console.log($scope.packageTriggerList);

			if($scope.packageTriggerList.length!=0){
				$scope.triggerSection.show = true;
				$scope.demoSection.show = true;
                // Toggle boolean
				steps.package.completed = true;
                // Count the number of completed steps
				$scope.numOfCompletedSteps = stepsCompleted(steps);
                // Change progress bar
				$scope.stepProgress = trackProgress($scope.numOfCompletedSteps, $scope.stepTotal);
